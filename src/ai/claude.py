...
[최우선 — Notion URL 룰]
메시지나 스레드 컨텍스트에 'notion.so' URL 또는 32자리 Notion ID(예: `36efd8bcf558806eaf26c8f982fd43d3`)가 들어있으면, 사용자가 명백히 "단순 조회"를 요청한 게 아닌 한 → **반드시 "notion_update"**. 다른 모든 분류보다 우선.
- notion_update로 보내야 하는 예 (URL/ID 존재 + 작성/맡김 의도):
  "여기에 추가해봐", "이거 정리해줘", "이번주 할일 적어줘", "여기다가 써줘",
  "확인하고 작성", "이 페이지에 업데이트해줘", "여기에 내 업무 정리"
- 예외 — 단순 조회로 두는 케이스 (이때만 briefing/general):
  "이 페이지 뭐야?", "이 페이지 누가 만들었어?", "이 페이지 내용 보여줘", "여기 어떤 데이터 있어?"
- 판단 기준: URL/ID + 행동 동사·맡김 어조 → notion_update, URL/ID + 단순 질문/호기심 어조 → briefing/general
- "할일", "이번주" 같은 단어가 같이 있어도 URL이 있고 작성 의도면 task_add/briefing이 아니라 notion_update
...
[작성 방향]
- 형식은 고정하지 않아요. 데이터 내용과 맥락에 맞게 자유롭게 구성하세요.
- 프로젝트 단위로 묶어서 그 프로젝트에서 한 작업을 정리하는 느낌으로 작성하세요.
- 너무 세세하게 단계별로 쪼개지 말고, 프로젝트별로 의미 있는 작업 덩어리 위주로 정리하세요.
- 핵심만 간결하게. 불필요한 수식어나 장황한 설명은 빼요.

[참고]
- 진행 중·예정 항목이 있으면 자연스럽게 포함하세요.
- 데이터에 없는 내용은 지어내지 마세요.
- markdown 문법(헤딩/리스트)으로만 표현하고, 표나 코드블록 같은 장식은 쓰지 마세요.
"""
