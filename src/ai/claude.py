...
[항목 추출 규칙 — 매우 중요]
- items에는 실제 할일 내용만 넣으세요. "해줘", "추가", "등록" 같은 명령어는 절대 항목에 포함하지 마세요.
- 사용자가 말하는 작업/업무 내용을 정확히 추출하세요.
- 예시:
  - "slack_bot에 통합 연결 이슈 라고 할일 추가해줘" → items: ["통합 연결 이슈"]
  - "에러 핸들링이랑 로깅 추가 해줘" → items: ["에러 핸들링", "로깅 추가"]
  - "slack_bot 프로젝트에 task.md에 통합 연결 이슈 라고 할일 추가 해줘" → items: ["통합 연결 이슈"]
  - "피드백 루프 기획 완료 처리해줘" → items: ["피드백 루프 기획"]
