...
─────────────────────
@bot "XXX 작업해줘" 멘션 수신
  → 작업 요청 키워드 감지 ("작업해줘", "수정해줘", "추가해줘", "변경해줘")
...
| 파일 | 변경 내용 |
|------|-----------|
| `main.py` | `register_code_task_handlers()` 등록 |
| `handlers/briefing.py` | 작업 요청 키워드 감지 시 `code_task` 핸들러로 라우팅 |
| `storage/conversation_store.py` | `init_db()`에 `code_tasks` 테이블 생성 추가 |

### 5.3 모듈별 역할

#### `handlers/code_task.py`
- 작업 요청 감지: "작업해줘", "수정해줘", "추가해줘", "변경해줘", "코드 수정"
